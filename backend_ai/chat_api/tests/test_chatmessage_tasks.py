...
class ChatMessageTasksTests(TestCase):
    ...
    @patch("chat_api.tasks.ChatMessage.objects.get")
    def test_ai_message_analysis_success(
        self, mock_get_message, mock_groq, mock_regress
    ):
    ...
    def test_ai_message_analysis_max_retries_failure(
        self, mock_release, mock_get_message, mock_retry
    ):
        mock_msg = MagicMock()
        mock_get_message.return_value = mock_msg
        ...
        with patch(
            "chat_api.tasks.groq_ai_insight_prompt", side_effect=Exception("API Down")
        ):
            # 1. Create a fake request context object
            mock_context = MagicMock()
            mock_context.retries = 3
            ai_message_analysis.max_retries = 3

            # 2. Safely push it onto Celery's request stack for the duration of the run
            ai_message_analysis.request_stack.push(mock_context)
            ...
            try:
                ...
                # 3. Always clean up the stack so it doesn't leak into other tests
                ai_message_analysis.request_stack.pop()
    ...
    def test_finalizer_aborted_signal(self):
        result = finalizer_task(
            "Stopped", self.session_id, self.message_id, self.user_lock_key
        )
    ...
    def test_finalizer_exception_handling(self, mock_release, mock_filter, mock_get):
        ...
        result = finalizer_task(
            {"data": "dummy"}, self.session_id, self.message_id, self.user_lock_key
        )
    ...
    def test_generate_ai_chat_response_full_logic(
        self, mock_chain, mock_get_message, mock_report_select
    ):
