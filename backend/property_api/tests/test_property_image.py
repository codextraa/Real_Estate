...
class PropertyImageTests(APITestCase):
    ...
    def _generate_test_image(
        self, name="test.jpg", size=(100, 100), color="white", format="JPEG"
    ):
        ...
        return SimpleUploadedFile(
            name, content=image_bytes.read(), content_type=f"image/{format.lower()}"
        )
    ...
    def test_upload_different_valid_formats(self):
        ...
        for name, fmt in formats:
            ...
            property_obj = Property.objects.filter(agent=self.agent_profile).latest(
                "id"
            )
    ...
    def test_update_property_image_success(self):
        ...
        response = self.client.patch(
            self.detail_url, {"property_image": new_image}, format="multipart"
        )
    ...
    def test_update_image_size_limit_fails(self):
        ...
        response = self.client.patch(
            self.detail_url, {"property_image": large_file}, format="multipart"
        )
    ...
    def test_update_unsupported_image_type_fails(self):
        ...
        response = self.client.patch(
            self.detail_url, {"property_image": gif_image}, format="multipart"
        )
